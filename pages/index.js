    ...
    <div className="">
      ...
      <main className="max-w-7xl mx-auto px-8 sm:px-16">
        <section className="py-6">
          ...
          {Array.isArray(exploreData) && exploreData.length>0 &&
            exploreData.map((elm) => (<SmallCard key={elm?.img} img={elm?.img} location={elm?.location} distance={elm?.distance} />))} 
          </div>
        </section>
        ...
        <section>
          ...
          <div className="flex space-x-3 overflow-scroll scrollbar-hide p-3 -ml-3">
            {
              Array.isArray(cardData) && cardData.length > 0 && cardData.map((elm) => (
